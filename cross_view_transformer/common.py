...
from cross_view_transformer.data.data_module import NuscenesDataModule, CarlaDataModule
from cross_view_transformer.losses import MultipleLoss
...
def setup_nuscenes_data_module(cfg: DictConfig) -> NuscenesDataModule:
    return NuscenesDataModule(cfg.data.dataset, cfg.data, cfg.loader)

def setup_carla_data_module(cfg: DictConfig) -> CarlaDataModule:
    return CarlaDataModule(cfg.data, cfg.loader)
...
def setup_experiment(cfg):
    ...
    data_module = setup_nuscenes_data_module(cfg)
    viz_fn = setup_viz(cfg)

    return model_module, data_module, viz_fn

def setup_carla_experiment(cfg):
    model_module = setup_model_module(cfg)
    data_module = setup_carla_data_module(cfg)
