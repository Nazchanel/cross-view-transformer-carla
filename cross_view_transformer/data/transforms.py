...
class SaveNuscenesDataTransform:
    """
...
class SaveCarlaDataTransform:
    @staticmethod
    def transform(batch):
        for key in batch:
            if not isinstance(batch[key], (str, list)):
                batch[key] = batch[key].tolist()
        return batch    

class LoadNuscenesDataTransform(torchvision.transforms.ToTensor):
...
class LoadCarlaDataTransform(torchvision.transforms.ToTensor):
    def __init__(self, image_cfg, augment='none'):
        super().__init__()
    ...
    def get_cameras(self, data_sample, h, w, top_crop, **image_cfg_kwargs):
        """
        Note: we invert I and E here for convenience
        """
        images_original = data_sample['image']
        intrinsics_original = data_sample['intrinsics']
        cam_ids = data_sample['cam_idx']
        extrinsics = data_sample['extrinsics']
        
        images = list()
        intrinsics = list()
        ImageFile.LOAD_TRUNCATED_IMAGES = True # prevents os error when loading truncated images

        for image_path, I_original in zip(images_original, intrinsics_original):
        ...
        return {
            'cam_idx': torch.LongTensor(cam_ids),
            'image': torch.stack(images, 0),
            'intrinsics': torch.stack(intrinsics, 0),
            'extrinsics': torch.tensor(np.float32(extrinsics)),
        }
    
    def get_bev(self, bev_image_path):
        bev = np.array(Image.open(bev_image_path))
        vehicles = mask(bev, (255, 0, 0))
        lane_markings = mask(bev, (0, 255, 0))
        drivable_area = mask(bev, (0, 0, 255))
        
        bev_masks = np.stack((vehicles, lane_markings, drivable_area))
        bev_masks = torch.tensor(bev_masks)
        
        return {'bev': bev_masks}

    def __call__(self, batch):
        result = {}
        result.update(self.get_bev(batch['bev']))
        result.update(self.get_cameras(batch, **self.image_cfg))
